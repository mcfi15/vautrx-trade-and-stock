...
@section('content')
    ...
    <div class="flex space-x-2 m-5">
        <a href="{{ route('admin.stocks.auto-import') }}"
    ...
    <div class="bg-blue-50 border-l-4 border-blue-500 text-blue-700 p-4 rounded-md mb-4 flex items-center">
        ...
        <p><strong>Live Updates:</strong> Stock prices update automatically every 30 seconds
            <span class="inline-block w-2 h-2 bg-green-500 rounded-full ml-2 animate-pulse"
                title="Live updates active"></span>
        </p>
    </div>
    @if(session('success'))
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div class="flex items-end space-x-2">
                        ...
                        <a href="{{ route('admin.stocks.index') }}"
                            class="inline-flex items-center px-3 py-2 border border-gray-300 text-gray-600 text-sm font-medium rounded-md hover:bg-gray-100">
                    ...
                    @forelse($stocks as $stock)
                        <tr class="border-b hover:bg-gray-50 transition" data-stock-symbol="{{ $stock->symbol }}">
                            <td class="px-4 py-3">
                                <div>
                                    <div class="font-semibold text-gray-800 flex items-center">
                                        {{ $stock->symbol }}
                                        <span class="ml-2 w-2 h-2 bg-green-500 rounded-full animate-pulse"
                                            title="Live updates enabled"></span>
                                    </div>
                                    <small class="text-gray-500">{{ $stock->name }}</small>
                                    @if($stock->sector)
                                        <div><span
                                                class="inline-block mt-1 px-2 py-0.5 bg-gray-100 text-gray-700 text-xs rounded-md">{{ $stock->sector }}</span>
                                        </div>
                                    @endif
                                </div>
                            </td>
                            ...
                            <td class="px-4 py-3">
                                @if($stock->change !== 0)
                                    <span
                                        class="px-2 py-0.5 rounded-full text-xs font-semibold {{ $stock->change >= 0 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                            ...
                            <td class="px-4 py-3">
                                <span
                                    class="px-2 py-0.5 rounded-full text-xs font-semibold {{ $stock->is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex justify-center space-x-2">
                                    <a href="{{ route('admin.stocks.show', $stock) }}" class="text-blue-500 hover:text-blue-700"
                                        title="View">
                                    ...
                                    <button onclick="syncStock({{ $stock->id }}, '{{ $stock->symbol }}')"
                                        class="text-indigo-500 hover:text-indigo-700" title="Sync">
